  ...
  document.getElementById("tc").innerHTML=`<div class="nut-wrap">
    <div class="nut-head">
      ...
      <div class="nut-nav-btns">
        ${date<isoToday()?`<button onclick="${ro?`unlockNut('${date}')`:`lockNut('${date}')`}" title="${ro?"Unlock editing":"Lock editing"}">${ro?`${icon("pencil",18)}`:`${icon("unlock",18)}`}</button>`:""}
...
// Streak added at 0.05, taken from deficit (0.55 -> 0.50) so the set still sums
...
function weekCompliance(p,weekStartIso,endIso){
  ...
  const m={
    ...
    zepbound:zepTaken,
    // A full unbroken week reads as 100. Rest days count toward the streak, so
    // seven is reachable without training seven days.
    streak:Math.min(100,Math.round((ctx.currentStreak?.().days||0)/7*100)),
  };
...
function phaseVerdict(p,finalAvg){
  ...
  return{band:"red",title:"Reassess",action:"Stop and reassess before pushing harder: review calorie adherence, food-logging accuracy, Apple Watch data, training consistency, sleep and recovery, Medication dose and response, and any injury or illness. Only then adjust calories, activity, or the timeline."};
}
...
function phaseCardHtml(){
  ...
  if(st==="completed"&&!run.locked){
    ...
    verdictHtml=`<div style="border:1px solid var(--b1);border-radius:12px;padding:12px 14px;margin-top:10px;background:var(--s2)">
      <div style="font-weight:700;font-size:14px;margin-bottom:4px"><span class="band-dot band-${v.band}"></span>${v.title} · final 7-day avg ${finalAvg} kg vs target ${p.targetKg} kg</div>
  ...
  const compHtml=comp.calculating
    ...
    :'<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:8px">'+
        ...
        compGridItem('Medication',comp.zepbound??0,true)+
        compGridItem('Streak',comp.streak??0)+
      '</div>'+
  ...
  return`<details class="st-acc"${st==="completed"?" open":""}>
    <summary><div><div>${icon("target",20)} ${p.id.replace("_"," ").replace(/^p/,"P")} · ${chipBits.filter(Boolean).join(" · ")}</div><div class="st-acc-sub">Day ${dayIn} of ${totalDays} · ${fmtD(p.start)} → ${fmtD(end)}${end!==p.plannedEnd?" (extended)":""} · ${p.startKg} → ${p.targetKg} kg · eat ${num(p.eatKcal)}/day</div></div></summary>
