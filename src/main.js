...
// Active program: V1 until May 31, V2 from June 1 2026 onwards
const PROG=new Date()>=new Date(2026,5,1)?PROG_V2:PROG_V1;

const DAYS=["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"];
const GYM="Stationary bike, treadmill, seated cable machine, chest press machine, pec fly machine, tricep extension machine, leg press (feet high 90 deg max), seated leg curl, seated hip abduction, seated calf raise, dumbbells 5-30kg";
...
const USER={birthDate:new Date(1995,7,1),weightKg:138,targetKg:90,heightCm:190.5,sex:"M",goalDate:new Date(2027,1,20)};
...
function requiredDeficit(lw,daysLeft){return Math.round(Math.max(0,(lw-USER.targetKg)*7700)/daysLeft);}
...
function noonUTC(iso){return new Date(iso+"T12:00:00Z");}
function daysBetween(aIso,bIso){return Math.round((noonUTC(bIso)-noonUTC(aIso))/86400000);}
function addDaysIso(iso,n){const d=noonUTC(iso);d.setUTCDate(d.getUTCDate()+n);return d.toISOString().split("T")[0];}
// Read-only accessor — never writes to S; mutations go through explicit actions
function getPhaseRun(id){return(S.phaseRun&&S.phaseRun[id])||{pauses:[],completedAt:null,outcome:null,locked:false};}
function ensurePhaseRun(id){S.phaseRun=S.phaseRun||{};S.phaseRun[id]=S.phaseRun[id]||{pauses:[],completedAt:null,outcome:null,locked:false};return S.phaseRun[id];}
// Whole days consumed by extend:true pauses (open pauses count up to `uptoIso`)
function pausedDaysExtend(p,uptoIso){
  let n=0;
...
function effectiveEnd(p,todayIso){return addDaysIso(p.plannedEnd,pausedDaysExtend(p,todayIso||isoToday()));}
// planned | active | paused | completed | locked
function phaseState(p,todayIso){
  ...
  if(t>effectiveEnd(p,t))return"completed"; // past end, awaiting verdict/lock
  return"active";
}
function phaseFor(dateIso){
...
function curveWeights(shape,nDays){
...
function phaseCurveKg(p,dateIso){
...
// ±1 kg tolerance corridor — UI language is "Target range today", never a point
function phaseCorridor(p,dateIso){const e=phaseCurveKg(p,dateIso);return{expected:e,lo:Math.round((e-1)*10)/10,hi:Math.round((e+1)*10)/10};}
function isRestDay(dateIso){return noonUTC(dateIso).getUTCDay()===0;} // Sunday
function phaseActiveTarget(p,dateIso){return isRestDay(dateIso)?p.activeTargetRest:p.activeTargetWorkout;}
// Background verification only (weekly deficit card, AI context) — never
// rendered on the Nutrition dashboard: users chase behaviours, not deficits.
function phaseDayDeficit(p,dateIso){return Math.round(p.restingKcal+ACTIVE_MULT*phaseActiveTarget(p,dateIso)-p.eatKcal);}
function restingFor(dateIso,dayData){
...
function phaseKgPerDay(p){const total=daysBetween(p.start,p.plannedEnd);return total?(p.startKg-p.targetKg)/total:0;}
// Ahead/behind the curve translated to schedule days (positive = ahead)
function bankedDays(p,todayIso){
...
// Trend projection to the overall 90kg goal, gated by weigh-in confidence so a
// few early water-loss days can't predict an unrealistically early finish.
function projectedFinish(todayIso){
...
// ── END PHASE ENGINE ─────────────────────────────────────────────────────────
const FIBRE_TARGET=38,SUGAR_LIMIT=50,SODIUM_LIMIT=2300,ACTIVE_MULT=0.75;
function calcAge(){const n=new Date();let a=n.getFullYear()-USER.birthDate.getFullYear();if(n<new Date(n.getFullYear(),USER.birthDate.getMonth(),USER.birthDate.getDate()))a--;return a;}
function calcBMR(w){return Math.round(10*w+6.25*USER.heightCm-5*calcAge()+5);}
function latestWeightLog(){const ws=S.nutrition.weights||{};const keys=Object.keys(ws).sort();return keys.length?ws[keys[keys.length-1]]:null;}
...
function calcTarget(bmr,active,dateIso){
  // Phase-aware: inside a phase the eating target is the phase's fixed kcal
  // and req is the phase's expected day deficit (background use only).
  // Outside any phase: date-driven fallback toward 90kg by Feb 20, 2027.
...
function isoDate(d){return d.toLocaleDateString("en-CA",{timeZone:"America/Toronto"});}
...
function getExUnit(key,exId){return S.sessions[key]?.[exId]?.unit||"kg";}
...
function renderW(){
  ...
  const stopped=!workoutOn&&!!(S.sessions[_key0]||{})._stopped;
  const lockedPast=isPastDay()&&!_unlocked[_key0];
  const readOnly=future||lockedPast||(stopped&&!isPastDay());
  const _vm=_viewMon||_todayMonday();
  ...
  if(future){
    ...
    const hasPlan=!!((S.weekPlans||{})[nextWk()]);
  ...
  if(!future&&!isPastDay()){
    ...
    if(allDone&&!stopped){
      ...
      const calfBtnHtml=workoutOn?`<button class="calf-btn${calfWarnCls}" id="calfBtn" onclick="logCalfTwinge()">⚡ Calf twinge<span class="calf-badge">${twinges}</span></button>${twinges>0?`<button class="btn-g" style="padding:8px 12px;font-size:11px" onclick="undoCalfTwinge()">Undo</button>`:""}`:"";
      h+=`<div class="session-bar">
        <button class="btn-start${workoutOn?" hide":""}" id="bStart" onclick="startSess()">⚡ Start Workout</button>
        <button class="btn-stop${workoutOn?" show":""}" id="bStop" onclick="stopSess()">■ Stop Workout</button>
        ${calfBtnHtml}
      </div>`;
    }
  }
  ...
  if(!isPastDay()&&!isFuture()&&_dowNow===0&&!(S.weekPlans||{})[nextWk()]){
...
function searchEx(q,day){
  ...
  const results=EX_DB
    ...
    .slice(0,8);
  if(!results.length){
    const banned=isBannedExercise(q);
  ...
  dd.innerHTML=results.map(({e},idx)=>{
    const banned=isBannedExercise(e.name);
    const btn=banned
...
function addFromDB(i,day){
  ...
  if(isBannedExercise(base.name)){suggestSafeAlt("db",day,i);return;}
  const sessKey=day+"_"+wk();
...
function addFreeform(day,name){
  ...
  if(!name)return;
  if(isBannedExercise(name)){suggestSafeAlt("free",day);return;}
  const sessKey=day+"_"+wk();
...
// Session
function startSess(){
  workoutOn=true;sessStart=Date.now();
...
function stopSess(){
  ...
  workoutOn=false;clearInterval(sessTimer);
  const el=Math.floor((Date.now()-sessStart)/1000);
  const m=Math.floor(el/60),s=el%60;
  const key=cDay+"_"+wk(); // always write to current week
  if(!S.sessions[key])S.sessions[key]={};
...
function submitLock(){
  ...
  if(!val){document.getElementById("lockErr").textContent="Enter your access token";return;}
  localStorage.setItem("forge_key",val);
  API_CFG.token=val;
...
function lockApp(){
  if(confirm("Lock the app? You'll need your access token to re-enter.")){
...
async function aiRun(type){
  ...
  if(type==="conv"){
    ...
    const todayExNames=(PROG[cDay]?.exercises||[]).map(e=>e.name||e).join(", ");
  ...
  try{
    const r=await fetch(API_CFG.baseUrl+"/api/coach",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+API_CFG.token},body:JSON.stringify({prompt,context:{day:cDay,program:PROG[cDay]?.title||""}})});
    const d=await r.json();
...
function canonicalId(exId){return _prCanonMap[exId]||exId.replace(/^(?:su2?|sa2?|f2?|th2?|w2?|t2?|m2?)_/,"");}
...
function currentStreak(){
  const today=isoToday();
  let n=0;
  for(let i=0;i<365;i++){const d=new Date(today+"T12:00:00");d.setDate(d.getDate()-i);if(trainedOn(isoDate(d)))n++;else break;}
  return n;
}
function checkMilestones(){
  ...
  if(!S.milestones.shownWeek6.includes(wkKey)){
    ...
    let weekTrains=0;
    for(let i=0;i<7;i++){const d=new Date(today+"T12:00:00");d.setDate(d.getDate()-i);const iso=isoDate(d);if(iso>=mon&&trainedOn(iso))weekTrains++;}
